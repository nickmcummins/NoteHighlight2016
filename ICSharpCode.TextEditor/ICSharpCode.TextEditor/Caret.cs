...
public class Caret : IDisposable
{
	...
	public Point ScreenPosition
	{
		get
		{
			...
			return checked(new Point(textArea.TextView.DrawingPosition.X + drawingXPos, textArea.TextView.DrawingPosition.Y + textArea.Document.GetVisibleLine(line) * textArea.TextView.FontHeight - textArea.TextView.TextArea.VirtualTop.Y));
		}
	}
	...
	{
		int lineNumber = Math.Max(0, Math.Min(checked(textArea.Document.TotalNumberOfLines - 1), pos.Y));
	...
	{
		line = Math.Max(0, Math.Min(checked(textArea.Document.TotalNumberOfLines - 1), line));
