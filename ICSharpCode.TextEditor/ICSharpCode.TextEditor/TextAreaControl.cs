...
public class TextAreaControl : Panel
{
	...
	private void AdjustScrollBarsClearCache()
	{
		...
		{
			if (lineLengthCache.Length < checked(Document.TotalNumberOfLines + 200))
			{
	...
	protected override void WndProc(ref Message m)
	{
		if (m.Msg == 123 && this.ShowContextMenu != null)
		{
			...
			if (num2 == -1 && num3 == -1)
			{
				...
				this.ShowContextMenu(this, new MouseEventArgs(MouseButtons.None, 0, screenPosition.X, checked(screenPosition.Y + textArea.TextView.FontHeight), 0));
			}
			else
			{
				Point screenPosition = PointToClient(new Point(num2, num3));
				this.ShowContextMenu(this, new MouseEventArgs(MouseButtons.Right, 1, screenPosition.X, screenPosition.Y, 0));
			}
		}
		base.WndProc(ref m);
	}
