	...
	private void PaintFoldMarker(Graphics g, int lineNumber, Rectangle drawingRectangle)
	{
		...
		checked
		{
			...
			if (flag)
			{
				...
				DrawFoldMarker(g, new RectangleF(drawingRectangle.X + unchecked(checked(drawingRectangle.Width - num) / 2), num2, num, num), flag7, flag4);
				if (flag2 || flag9)
				{
			...
			else if (flag3)
			{
				...
				g.DrawLine(BrushRegistry.GetPen(flag6 ? colorFor2.Color : colorFor.Color), num3, num4, num3 + unchecked(num / 2), num4);
