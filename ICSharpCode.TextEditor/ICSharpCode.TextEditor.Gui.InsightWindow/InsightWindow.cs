...
public class InsightWindow : AbstractCompletionWindow
{
	...
	protected override bool ProcessTextAreaKey(Keys keyData)
	{
		...
		case Keys.Down:
			...
			{
				CurrentData = checked(CurrentData + 1) % DataProvider.InsightDataCount;
				Refresh();
			}
			return true;
		case Keys.Up:
			if (DataProvider != null && DataProvider.InsightDataCount > 0)
			{
				CurrentData = checked(CurrentData + DataProvider.InsightDataCount - 1) % DataProvider.InsightDataCount;
				Refresh();
			}
			return true;
		default:
	...
	protected override void CaretOffsetChanged(object sender, EventArgs e)
	{
		...
		checked
		{
			int num = (1 + position.Y) * control.ActiveTextAreaControl.TextArea.TextView.FontHeight - control.ActiveTextAreaControl.TextArea.VirtualTop.Y - 1 + control.ActiveTextAreaControl.TextArea.TextView.DrawingPosition.Y;
			int drawingXPos = control.ActiveTextAreaControl.TextArea.TextView.GetDrawingXPos(position.Y, position.X);
			int num2 = (control.ActiveTextAreaControl.Document.GetVisibleLine(position.Y) + 1) * control.ActiveTextAreaControl.TextArea.TextView.FontHeight - control.ActiveTextAreaControl.TextArea.VirtualTop.Y;
			int num3 = (control.TextEditorProperties.ShowHorizontalRuler ? control.ActiveTextAreaControl.TextArea.TextView.FontHeight : 0);
			Point location = control.ActiveTextAreaControl.PointToScreen(new Point(drawingXPos, num2 + num3));
	...
	protected override void OnPaint(PaintEventArgs pe)
	{
		...
		else
		{
			...
			{
				countMessage = control.GetRangeDescription(checked(CurrentData + 1), DataProvider.InsightDataCount);
			}
