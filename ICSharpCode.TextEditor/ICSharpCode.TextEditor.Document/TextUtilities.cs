...
public sealed class TextUtilities
{
	...
	public static int FindNextWordStart(IDocument document, int offset)
	{
		...
		checked
		{
			int num2 = lineSegmentForOffset.Offset + lineSegmentForOffset.Length;
			CharacterType characterType = GetCharacterType(document.GetCharAt(offset));
			while (offset < num2 && GetCharacterType(document.GetCharAt(offset)) == characterType)
			{
				offset++;
			}
			while (offset < num2 && GetCharacterType(document.GetCharAt(offset)) == CharacterType.WhiteSpace)
			{
				offset++;
			}
			return offset;
