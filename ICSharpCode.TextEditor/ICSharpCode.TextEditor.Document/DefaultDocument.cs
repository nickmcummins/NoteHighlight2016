...
internal sealed class DefaultDocument : IDocument
{
	...
	public TextLocation OffsetToPosition(int offset)
	{
		...
		return new TextLocation(checked(offset - lineSegment.Offset), lineNumberForOffset);
	}
	...
	public int PositionToOffset(TextLocation p)
	{
		...
		return Math.Min(TextLength, checked(lineSegment.Offset + Math.Min(lineSegment.Length, p.X)));
	}
