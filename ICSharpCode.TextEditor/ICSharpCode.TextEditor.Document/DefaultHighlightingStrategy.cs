...
public class DefaultHighlightingStrategy : IHighlightingStrategyUsingRuleSets, IHighlightingStrategy
{
	...
	private bool MarkTokensInLine(IDocument document, int lineNumber, ref bool spanChanged)
	{
		...
		bool result = false;
		LineSegment lineSegment = ((lineNumber > 0) ? document.GetLineSegment(checked(lineNumber - 1)) : null);
		currentSpanStack = ((lineSegment != null && lineSegment.HighlightSpanStack != null) ? lineSegment.HighlightSpanStack.Clone() : null);
		if (currentSpanStack != null)
		{
			while (!currentSpanStack.IsEmpty && currentSpanStack.Peek().StopEOL)
			...
			for (int i = 0; i < length; i++)
			{
				...
				if (activeRuleSet != null)
				{
					foreach (Span span in activeRuleSet.Spans)
					{
						...
						PushCurWord(document, ref markNext, list);
						string regString = GetRegString(currentLine, span.Begin, i, document);
						if (!OverrideSpan(regString, document, list, span, ref i))
						{
							currentLength += regString.Length;
				...
				if (activeRuleSet != null && charAt < 'Ā' && activeRuleSet.Delimiters[unchecked((uint)charAt)])
				{
					PushCurWord(document, ref markNext, list);
	...
	private void PushCurWord(IDocument document, ref HighlightColor markNext, List<TextWord> words)
	{
		...
		checked
		{
			...
			{
				HighlightColor highlightColor = ((markNext != null) ? markNext : GetColor(activeRuleSet, document, currentLine, currentOffset, currentLength));
				if (highlightColor == null)
				{
				...
				{
					words.Add(new TextWord(document, currentLine, currentOffset, currentLength, highlightColor, hasDefaultColor: false));
				}
			}
			if (activeRuleSet != null)
			{
		...
		checked
		{
			while (num < expr.Length)
			{
				...
				if (c == '@')
				{
					...
					if (num < expr.Length)
					{
						...
						case '-':
						{
							...
							for (i = 0; i < stringBuilder.Length; i++)
							{
								char c2 = (ignoreCase ? char.ToUpperInvariant(document.GetCharAt(lineSegment.Offset + index - stringBuilder.Length + i)) : document.GetCharAt(lineSegment.Offset + index - stringBuilder.Length + i));
								char c3 = (ignoreCase ? char.ToUpperInvariant(stringBuilder[i]) : stringBuilder[i]);
								if (c2 != c3)
								{
									break;
				...
				else
				{
					...
					char c2 = (ignoreCase ? char.ToUpperInvariant(document.GetCharAt(lineSegment.Offset + index + num2)) : document.GetCharAt(lineSegment.Offset + index + num2));
					char c3 = (ignoreCase ? char.ToUpperInvariant(expr[num]) : expr[num]);
					if (c2 != c3)
					{
						return false;
					}
