...
internal sealed class AugmentableRedBlackTree<T, Host> : ICollection<T>, IEnumerable<T>, IEnumerable where Host : IRedBlackTreeHost<T>
{
	...
	private void CheckProperties()
	{
		...
		CheckNodeProperties(root, null, parentColor: true, 0, ref expectedBlackCount);
		int num = 0;
	...
	private void CheckNodeProperties(RedBlackTreeNode<T> node, RedBlackTreeNode<T> parentNode, bool parentColor, int blackCount, ref int expectedBlackCount)
	{
		...
		if (parentColor)
		{
			Debug.Assert(!node.color);
		}
		if (!node.color)
		{
			blackCount = checked(blackCount + 1);
		}
	...
	private void FixTreeOnInsert(RedBlackTreeNode<T> node)
	{
		...
		Debug.Assert(node.color);
		Debug.Assert(node.left == null || !node.left.color);
		Debug.Assert(node.right == null || !node.right.color);
		...
		else
		{
			...
			Debug.Assert(node == parent.right && parent == parent2.right);
			RotateLeft(parent2);
		}
	}
	...
	private void FixTreeOnDelete(RedBlackTreeNode<T> node, RedBlackTreeNode<T> parentNode)
	{
		...
		if (node == parentNode.left)
		{
			if (redBlackTreeNode.right != null)
			{
				Debug.Assert(redBlackTreeNode.right.color);
				redBlackTreeNode.right.color = false;
			}
			RotateLeft(parentNode);
		}
		else
		{
			if (redBlackTreeNode.left != null)
			{
				Debug.Assert(redBlackTreeNode.left.color);
				redBlackTreeNode.left.color = false;
			}
			RotateRight(parentNode);
		}
