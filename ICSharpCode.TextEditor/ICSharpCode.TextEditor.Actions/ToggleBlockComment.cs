	...
	public override void Execute(TextArea textArea)
	{
		...
		if (text != null && text.Length != 0 && text2 != null && text2.Length != 0)
		{
			...
			if (textArea.SelectionManager.HasSomethingSelected)
			{
				...
				num = textArea.SelectionManager.SelectionCollection[checked(textArea.SelectionManager.SelectionCollection.Count - 1)].EndOffset;
			}
			else
			{
	...
	public static BlockCommentRegion FindSelectedCommentRegion(IDocument document, string commentStart, string commentEnd, int selectionStartOffset, int selectionEndOffset)
	{
		...
		checked
		{
			...
			if (num2 == -1)
			{
				...
				if (num2 >= 0)
				{
					num3 = text2.IndexOf(commentEnd, num2, selectionStartOffset - num2);
					if (num3 > num2)
					{
						num2 = -1;
					}
			...
			if (num == -1)
			{
				...
				string text2 = document.GetText(num4, document.TextLength - num4);
				num = text2.IndexOf(commentEnd);
				if (num >= 0)
				{
					num += num4;
				}
