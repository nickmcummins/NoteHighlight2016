	...
	public override void Execute(TextArea textArea)
	{
		checked
		{
			...
			else
			{
				...
				string text = textArea.Document.GetText(lineSegmentForOffset.Offset, textArea.Caret.Offset - lineSegmentForOffset.Offset);
				int indentationSize = textArea.Document.TextEditorProperties.IndentationSize;
				int column = textArea.Caret.Column;
				int num = unchecked(column % indentationSize);
				if (num == 0)
				{
